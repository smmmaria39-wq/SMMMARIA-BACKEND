...
export const settlePayment = async (
  ...
  paymentId = String(paymentId).replace(/^payments\//, "");
  ...
  if (paymentData.status === "completed" || paymentData.status === "approved") {
    logger.info(
      `[Settlement] DUPLICATE PREVENTED: Payment ${paymentId} is already completed/approved.`,
    );
  ...
  if (paymentData.status === "cancelled" || paymentData.status === "rejected") {
    logger.warn(
      `[Settlement] Attempt to settle cancelled/rejected payment ${paymentId}. Aborted.`,
    );
  ...
  if (paymentData.gatewayReference) {
    ...
    if (!gwClaimResult.committed) {
      logger.warn(
        `[Settlement] Duplicate gateway reference ${paymentData.gatewayReference} detected for payment ${paymentId}. Rejecting.`,
      );
      ...
      if (paymentData.userId) {
        try {
          await getRef(`users/${paymentData.userId}/activeDeposit`).remove();
        } catch (_) {}
      }
      return {
        success: false,
        message: "Duplicate gateway transaction prevented",
      };
    }
  }
  ...
  if (paymentData.status === "pending") {
    ...
    if (!claimRes.committed)
      return { success: false, message: "Payment claimed by another process" };
    ...
    if (!takeoverRes.committed)
      return {
        success: false,
        message: "Payment completed by another process or no longer stale",
      };
    logger.warn(
      `[Settlement] Stale processing detected for ${paymentId}. Taking over via ${source}.`,
    );
  ...
  let totalCreditUSD =
    paymentData.totalCreditUSD ||
    paymentData.totalCredit ||
    parseFloat(paymentData.amount) + parseFloat(paymentData.bonus || 0);
  ...
  if (!Number.isFinite(totalCreditUSD) || totalCreditUSD <= 0) {
    logger.error(
      `[Settlement] Invalid credit amount for ${paymentId}: ${totalCreditUSD}. Rejecting.`,
    );
    await paymentRef.update({
      status: "rejected",
      failureReason: "Invalid credit amount during settlement",
    });
    await getRef(`transactions/${paymentId}`).update({ status: "rejected" });
    if (userId) {
      try {
        await getRef(`users/${userId}/activeDeposit`).remove();
      } catch (_) {}
    }
    return {
      success: false,
      message: "Invalid credit amount prevented settlement",
    };
  }
  ...
  if (!creditRes.committed) {
    ...
    if (!userVal) {
      ...
      logger.error(
        `[Settlement] User record missing for ${paymentId}. Reverted to pending.`,
      );
      throw new Error(
        "User record missing during settlement. Payment reverted to pending.",
      );
    }
    ...
    if (userVal.walletCredits && userVal.walletCredits[paymentId]) {
      ...
      logger.info(
        `[Settlement] Payment ${paymentId} was already credited. Finalizing status.`,
      );
    } else {
      ...
      logger.warn(
        `[Settlement] Concurrent user update for ${paymentId}. Reverted to pending.`,
      );
      throw new Error(
        "Concurrent user update during settlement. Payment reverted to pending.",
      );
    }
  }
  ...
  try {
    await getRef(`walletLedger/${paymentId}`).set({
      ...
      gateway:
        paymentData.gateway ||
        (paymentData.method === "card" ? "marzpay" : "pesajet"),
    ...
    logger.error(
      `[Settlement] Failed to write to top-level walletLedger for ${paymentId}. Idempotency is safe, but audit log may be missing.`,
      e,
    );
  }
  ...
  if (paymentData.idempotencyKey) {
    await getRef(
      `paymentIdempotency/${userId}/${paymentData.idempotencyKey}`,
    ).update({ status: "completed" });
  }

  // Clear activeDeposit lock for this user so they are not blocked from future deposits
  if (userId) {
    try {
      await getRef(`users/${userId}/activeDeposit`).remove();
    } catch (lockErr) {
      logger.warn(
        `[Settlement] Failed to remove activeDeposit lock for user ${userId}: ${lockErr.message}`,
      );
    }
  }

  logger.success(
    `[Settlement] Payment ${paymentId} settled successfully via ${source}. Credited $${totalCreditUSD}.`,
  );
