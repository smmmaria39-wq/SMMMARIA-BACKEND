  ...
  if (method === 'manual') {
   ...
   return successResponse(res, 'Deposit request created. Awaiting admin approval.', paymentData, 201);
    ...
    try {
     ...
     // Construct payload for WearAmaze based on exact documentation requirements
     let gatewayPayload = {
      amount: amountInUGX.toString(), // API expects string
      country: "UG", // Hardcoded Uganda country code
      reference: `SMMMARIA-${paymentId.substring(0, 8)}`, // Unique reference
      description: "Wallet Deposit", // Auto-filled description
      callback_url: "https://smmaria.netlify.app/api/v1/payments/webhook" // Required callback URL
     };
